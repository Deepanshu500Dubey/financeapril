...
class TrialBalanceRequest(BaseModel):
    fiscal_period: str = Field(..., example="2026-08")
...
class YoYComparisonRequest(BaseModel):
    current_period: str = Field(..., example="2026-08")
...
class EmailReportRequest(BaseModel):
    ...
    fiscal_period: str = Field("2026-08", description="Fiscal period for reports")
...
class ApprovalRegistry:
    ...
    def register_approval(self, token: str, approval_item: ApprovalItem):
        ...
        self.generated_approvals[token] = {
            ...
            'fiscal_period': approval_item.metadata.get('fiscal_period', '2026-08') if approval_item.metadata else '2026-08',
...
def update_progress_from_approvals():
    ...
    update_milestones_from_approvals("2026-08")
...
def update_milestones_from_approvals(fiscal_period: str = "2026-08"):
    """
...
def analyze_close_readiness(fiscal_period: str = "2026-08") -> Dict[str, Any]:
    """
...
def generate_trial_balance_data(fiscal_period: str, entity_code: str) -> Dict[str, Any]:
    ...
    try:
        # Load data
        transactions = load_csv_data('Raw_GL_Export_With_CostCenters_Aug2026.csv')
        coa = load_coa()
        ar_records = load_csv_data('AR_Subledger_Aug2026.csv')
...
def generate_pdf_report(
    ...
    try:
        ...
        coa = load_coa()
        ar_records = load_csv_data('AR_Subledger_Aug2026.csv')
        budget_data = load_csv_data('Budget_Aug2026_Detailed.csv')
...
async def decide_approval(
    ...
    history_record['fiscal_period'] = item.metadata.get('fiscal_period', '2026-08') if item.metadata else '2026-08'
...
def assign_cost_centers(request: CostCenterBatchRequest):
    ...
    try:
        ...
        if updated_count > 0:
            ...
            save_csv_data('Raw_GL_Export_With_CostCenters_Aug2026.csv', transactions, fieldnames)
...
async def cfo_financial_dashboard(
    fiscal_period: str = Query("2026-08", description="Fiscal period to display"),
...
async def send_test_email(
    background_tasks: BackgroundTasks,
    fiscal_period: str = Query("2026-08"),
...
class IntercompanyReconciliationRequest(BaseModel):
    fiscal_period: str = Field("2026-08", description="Fiscal period for reconciliation")
...
class IntercompanyEliminationRequest(BaseModel):
    fiscal_period: str = Field("2026-08", description="Fiscal period")
...
async def intercompany_reconcile(request: IntercompanyReconciliationRequest):
    ...
    try:
        # Load data
        transactions = load_csv_data('Intercompany_Transactions_Aug2026.csv')
        reconciliation = load_csv_data('Intercompany_Reconciliation_Aug2026.csv')
        elimination_journals = load_csv_data('Intercompany_Elimination_Journals_Aug2026.csv')
...
async def resolve_intercompany_variance(request: IntercompanyVarianceResolution):
    ...
    try:
        ...
        save_csv_data('Intercompany_Reconciliation_Aug2026.csv', reconciliation, fieldnames)
...
class AccrualsPrepaymentsRequest(BaseModel):
    fiscal_period: str = Field("2026-08", description="Fiscal period for analysis")
...
async def analyze_accruals_prepayments(request: AccrualsPrepaymentsRequest):
    ...
    try:
        # Load data
        accruals = load_csv_data('Accruals_Register_Aug2026.csv')
        prepayments = load_csv_data('Prepayments_Register_Aug2026.csv')
        adjustment_journals = load_csv_data('Accrual_Adjustment_Journals_Aug2026.csv')
        amortization_journals = load_csv_data('Prepayment_Amortization_Journals_Aug2026.csv')
...
async def post_accrual_adjustments(request: AccrualAdjustmentRequest):
    ...
    try:
        ...
        save_csv_data('Accrual_Adjustment_Journals_Aug2026.csv', adjustment_journals, fieldnames)
...
async def bank_reconciliation(request: BankReconciliationRequest):
    ...
    try:
        # Load data
        bank_statements = load_csv_data('Bank_Statements_Aug2026.csv')
        gl_cash_balances = load_csv_data('GL_Cash_Balances_Aug2026.csv')
        reconciliation_items = load_csv_data('Bank_Reconciliation_Items_Aug2026.csv')
        reconciliation_journals = load_csv_data('Bank_Reconciliation_Journals_Aug2026.csv')
...
async def post_bank_reconciliation_journals(
    ...
    try:
        ...
        if updated_count > 0:
            ...
            save_csv_data('Bank_Reconciliation_Journals_Aug2026.csv', reconciliation_journals, fieldnames)
...
async def resolve_bank_reconciliation_item(request: BankReconciliationItemResolution):
    ...
    try:
        ...
        save_csv_data('Bank_Reconciliation_Items_Aug2026.csv', reconciliation_items, fieldnames)
...
async def get_close_status(fiscal_period: str = Query("2026-08")):
    """
    Get combined status of all close activities
    """
    try:
        # Load all data
        ic_transactions = load_csv_data('Intercompany_Transactions_Aug2026.csv')
        ic_reconciliation = load_csv_data('Intercompany_Reconciliation_Aug2026.csv')
        accruals = load_csv_data('Accruals_Register_Aug2026.csv')
        prepayments = load_csv_data('Prepayments_Register_Aug2026.csv')
        bank_items = load_csv_data('Bank_Reconciliation_Items_Aug2026.csv')
...
IBMBOB_DATA_FILES = {
    'pl': (
        'IBMBOB_Group_PL_LineItems_Aug2026.csv',
        'IBMBOB_Group_PL_LineItems_Aug2026_GENERATED.csv',
    ),
    'workforce': (
    ...
    'esg': (
        'IBMBOB_ESG_Cost_KPI_Aug2026.csv',
        'IBMBOB_ESG_Cost_KPI_Aug2026_GENERATED.csv',
    ),
    'enhanced_gl': (
        'Raw_GL_Export_With_CostCenters_Aug2026_IBMBOB_Enhanced.csv',
        'Raw_GL_Export_With_CostCenters_Aug2026_IBMBOB_Enhanced_GENERATED.csv',
    ),
}
...
async def generate_pl_statement(fiscal_period: str = Query("2026-08")):
    """
...
async def get_pl_transactions(fiscal_period: str = Query("2026-08"), section: Optional[str] = Query(None)):
...
async def get_pl_approval_items(fiscal_period: str = Query("2026-08")):
...
async def reconcile_pl_to_gl(fiscal_period: str = Query("2026-08")):
...
async def analyze_pl_variances(fiscal_period: str = Query("2026-08")):
...
async def analyze_workforce_cost_revenue(fiscal_period: str = Query("2026-08"), business_unit: Optional[str] = Query(None)):
...
async def analyze_salary_costs(fiscal_period: str = Query("2026-08")):
...
async def analyze_workforce_efficiency(fiscal_period: str = Query("2026-08")):
...
async def optimize_resource_allocation(fiscal_period: str = Query("2026-08")):
...
async def calculate_labour_cost_metrics(fiscal_period: str = Query("2026-08"), business_unit: Optional[str] = Query(None)):
...
async def detect_esg_leakage(fiscal_period: str = Query("2026-08")):
...
async def analyze_esg_financial_impact(fiscal_period: str = Query("2026-08")):
...
async def assess_disclosure_readiness(fiscal_period: str = Query("2026-08")):
...
async def generate_board_narrative(fiscal_period: str = Query("2026-08")):
...
async def track_esg_kpis(fiscal_period: str = Query("2026-08"), category: Optional[str] = Query(None)):
...
async def close_progress_dashboard(fiscal_period: str = Query("2026-08")):
    """
        ...
        <div class="header">
            ...
            <div class="period-control">
                <select id="periodSelect" class="period-select">
                    <option value="2026-08" {'selected' if fiscal_period == '2026-08' else ''}>August 2026</option>
...
async def get_close_progress_api(fiscal_period: str = Query("2026-08")):
    """
...
async def startup_event():
    ...
    required_files = [
        'Raw_GL_Export_With_CostCenters_Aug2026.csv',
        'Master_COA_Complete.csv',
        'Master_CostCenters_States.csv',
        'AR_Subledger_Aug2026.csv',
        'Budget_Aug2026_Detailed.csv',
        'PL_Statement_Mar2025_Comparative.csv',
        'Intercompany_Transactions_Aug2026.csv',
        'Intercompany_Reconciliation_Aug2026.csv',
        'Intercompany_Elimination_Journals_Aug2026.csv',
        'Accruals_Register_Aug2026.csv',
        'Prepayments_Register_Aug2026.csv',
        'Accrual_Adjustment_Journals_Aug2026.csv',
        'Prepayment_Amortization_Journals_Aug2026.csv',
        'Bank_Statements_Aug2026.csv',
        'GL_Cash_Balances_Aug2026.csv',
        'Bank_Reconciliation_Items_Aug2026.csv',
        'Bank_Reconciliation_Journals_Aug2026.csv'
    ]
...
function showDownloads(data) {
  ...
  container.innerHTML = `
    ...
    <div class="download-card">
      ...
      <div class="download-desc">Monthly Aug/Aug/Sep driver model</div>
