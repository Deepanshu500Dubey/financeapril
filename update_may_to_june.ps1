# Script to update May date references in CSV files to June
# This will copy May CSV files and update them for June closing

$sourceDir = "."  # Use current folder

# List of May CSV files to convert to June
$mayFiles = @(
    "IBMBOB_Workforce_Cost_Output_May2026.csv"
)

# Additional files that contain May data that need June versions
# These are the April files that you want to convert to June
$filesToCopy = @(
...
Write-Host "Starting conversion from May to June 2026 data..." -ForegroundColor Green
Write-Host "Source directory: $sourceDir" -ForegroundColor Cyan

# Copy May files and update to June
foreach ($file in $mayFiles) {
    $sourcePath = Join-Path $sourceDir $file
    $destPath = Join-Path $sourceDir ($file -replace 'May2026', 'Jun2026')
    ...
    if (Test-Path $sourcePath) {
        ...
        $content = $content -replace 'May_2026', 'Jun_2026'
...
# Create June versions from existing May files
foreach ($file in $filesToCopy) {
    ...
    if (Test-Path $sourcePath) {
        ...
        # Create new filename for June
        $newFileName = $file -replace 'May2026', 'Jun2026'
        $destPath = Join-Path $sourceDir $newFileName
        
        Set-Content -Path $destPath -Value $content -NoNewline
        Write-Host "Created June version: $newFileName"
    }
    else {
        Write-Host "File not found: $file" -ForegroundColor Yellow
    }
...
Write-Host "`nAll files updated successfully for June 2026 closing!" -ForegroundColor Green
Write-Host "Next: Update app.py to reference Jun2026 files instead of May2026"